...
class HttpClient {
  ...
  private readonly handlers: RequestHandlers | undefined
  ...
  private requestOnFulfilled(
    ...
    // 处理请求头，例如添加 token
    if (this.handlers?.handleRequestHeader) {
  ...
  private setupInterceptors(interceptors?: HttpClientInterceptors): void {
    ...
    this.instance.interceptors.request.use(
      interceptors?.requestOnFulfilled || this.requestOnFulfilled,
      interceptors?.requestOnRejected || this.requestOnRejected
    )

    // 响应拦截器
    this.instance.interceptors.response.use(
      interceptors?.responseOnFulfilled || this.responseOnFulfilled,
      interceptors?.responseOnRejected || this.responseOnRejected
    )
  }
